...
export class WalkToAction extends Action {
  ...
  enter() {
    ...
    if (path && path.length > 0) { this._sim.startPath(path); }
    else { Logger.warn(`[WalkTo] No path to (${this._gx},${this._gz})`); this.done = true; }
  }
...
export class UseObjectAction extends Action {
  constructor(sim, furniture, duration = 5) {
    ...
    this._sim = sim; this._furniture = furniture; this._duration = duration; this._elapsed = 0;
  }
