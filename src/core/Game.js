...
export class Game {
  ...
  _init() {
    ...
    window._game = this;
    Logger.info('Game ready — doors, social, object registry');
  }
  ...
  _update(dt) {
    ...
    this.__camera.follow(new THREE.Vector3(this._selectedSim.worldX, 0, this._selectedSim.worldZ));
  ...
  _onCanvasClick(e) {
    ...
    if (simHits.length > 0) {
      ...
      const idx = this._sims.findIndex(s => s.mesh === hit || s.mesh === hit.parent);
